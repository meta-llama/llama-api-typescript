...
describe('logging', () => {
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new LlamaAPIClient({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'LLAMA_API_CLIENT_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new LlamaAPIClient({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new LlamaAPIClient({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual('http://localhost:5000/client/foo');
    });

    test('in request options overridden by env variable', () => {
      process.env['LLAMA_API_CLIENT_BASE_URL'] = 'http://localhost:5000/env';
      const client = new LlamaAPIClient({ apiKey: 'My API Key' });
