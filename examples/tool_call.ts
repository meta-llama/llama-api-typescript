...
import { CompletionCreateParams, CreateChatCompletionResponse } from 'llama-api/resources/chat';
...
async function run_streaming(): Promise<void> {
    ...
    for await (const chunk of response) {
        ...
        if (chunk.event.delta.type === 'tool_call' && chunk.event.delta.function?.name) {
            console.log(
                `Using tool_id=${chunk.event.delta.id} with name=${chunk.event.delta.function.name}`,
            );
